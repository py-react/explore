...
const useState = (initialValue) => {
  const state = {
    value: initialValue.on ? initialValue : cloneItem(
  ...
  const proxy = new Proxy(state, {
    set(target, prop, value) {
      ...
      if(prop !== "value") throw new Error("Direct mutation is not allowed");
      if(!value.fromSetState) throw new Error("Direct mutation is not allowed");
      if(prop in target && isEqual(target[prop],value.data)) return false 
  ...
  const setState = (newValue) => {
      try {
        if (typeof newValue === "function") {
          proxy.value = {
            fromSetState:true,
            data:newValue(proxy.value)
          };
        } else {
          proxy.value = {
            fromSetState:true,
            data:cloneItem(newValue)
          };
        }
      } catch (error) {
